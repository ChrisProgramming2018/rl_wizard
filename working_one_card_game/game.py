...
def evalgame(game, agent1_act, agent2_act, agent1_est, agent2_est,  cards, episode=3):
    #game = Game(args, deck)
    for epi in range(episode):
        game.reset_round(cards)
        for i in range(1, cards + 1):
            state_agent1, state_agent2 = game.create_state_vector(1)
            estimate1 =  agent1_est[i-1].act(state_agent1, 0, True) 
            estimate2 = agent2_est[i-1].act(state_agent2,0, True)
            game.set_estimate(estimate1, estimate2)
            print("Estimate Player 1 predict: {}  with card {} ".format(estimate1, game.player1.current_cards))
            print("Estimate Player 2 predict: {}  with card {} ".format(estimate2, game.player2.current_cards))
            print("Trumph card {} ".format(game.current_trumph))
...
def main(args):
    ...
    agent2_action_mean = []
    game = Game(args, deck)
    evalgame(game, agent1_act, agent2_act, agent1_est, agent2_est, 1)
    ...
    for ep in range(epochs):
        ...
        for i in range(1,2):
            ...
            while game.play_game:
                ...
                game.play_cards(action_1, action_2)
                next_state_agent1, next_state_agent2 = game.create_state_vector(0)
                buffer_1.append([state_agent1, action_1, next_state_agent1, done])
                buffer_2.append([state_agent2, action_2, next_state_agent2, done])
                buffer_1_est.append([state_agent1, estimate1, next_state_agent1, done])
                buffer_2_est.append([state_agent2, estimate2, next_state_agent2, done])
                state_agent1 = next_state_agent1
                state_agent2 = next_state_agent2
            game.evalute_game()
            buffer_1, buffer_2 = game.set_buffer(buffer_1_est, buffer_2_est, writer, total_time_steps)
            ...
            if total_time_steps % 1000 == 0:
                # save model 
                for agent in agent1_est:
                    file_name = path + "Est_Agent_" + str(i) 
                    file_name += "_epoch_" + str(ep)
                    file_name += "_mean_reward_" + str(mean1) 
            ...
            i = 0
            for traj1, traj2 in zip(buffer_1_est, buffer_2_est):
